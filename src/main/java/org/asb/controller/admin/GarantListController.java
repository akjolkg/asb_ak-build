...
import org.asb.data.GarantDataModel;
...
public class GarantListController implements Serializable {
	
	private static final long serialVersionUID = 8475958315897562353L;
	private GarantDataModel model; 
	...
	public void filterData(){
		...
		model = new GarantDataModel(filters, service);
		
	}
	
	
	public GarantDataModel getModel() {
		return model;
	}
	
	public void setModel(GarantDataModel model) {
		this.model = model;
	}
	...
	public void restoreState() {
    	HttpServletRequest request = (HttpServletRequest)FacesContext.getCurrentInstance().getExternalContext().getRequest();
		HttpSession session = request.getSession();
		model = (GarantDataModel) session.getAttribute("model");
